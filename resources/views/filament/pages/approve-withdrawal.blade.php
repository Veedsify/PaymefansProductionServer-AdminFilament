<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div class="p-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 shadow-sm dark:border-gray-700">
            <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Withdrawal Details</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="font-semibold text-gray-900 dark:text-white">₦{{ number_format($record->amount) }}</p>
                </div>
                ...
                <div>
                    ...
                    <p class="font-semibold text-green-600 dark:text-green-400">₦{{ number_format(\App\Services\PaystackService::calculateAmountAfterFee($record->amount)) }}</p>
                </div>
        ...
        <div class="p-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 shadow-sm dark:border-gray-700">
            <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Approve Withdrawal</h3>
            ...
            <div class="space-y-4">
                ...
                <button wire:click="initiateTransfer" wire:loading.attr="disabled" wire:target="initiateTransfer" type="button" class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        @if($transferInitiated && $record->status !== 'completed' && $record->status !== 'rejected')
        <div class="p-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 shadow-sm dark:border-gray-700">
            <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Enter OTP</h3>
            ...
            <div class="space-y-4">
                ...
                <div class="p-4 rounded-lg bg-gray-50 dark:bg-gray-900">
                    <div class="text-sm grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-mono text-gray-900 dark:text-white">{{ $transferCode }}</p>
                        </div>
                        <div>
                            <label class="block font-medium text-gray-500 dark:text-gray-400">Reference</label>
                            <p class="font-mono text-gray-900 dark:text-white">{{ $reference }}</p>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <form wire:submit="finalizeTransfer">
                    <div class="space-y-4">
                        <!-- OTP Input Field -->
                        <div>
                            <label for="otp" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                                OTP Code
                            </label>
                            <input wire:model.live="data.otp" type="text" id="otp" name="otp" maxlength="6" placeholder="Enter 6-digit OTP" class="w-full px-3 py-2 text-2xl tracking-widest text-center border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white" autocomplete="off" required x-data="" x-init="$nextTick(() => $el.focus())" x-on:input="$el.value = $el.value.replace(/[^0-9]/g, '')" />
                        ...
                        <button type="submit" wire:loading.attr="disabled" wire:target="finalizeTransfer" x-data="{ otpValid: false }" x-init="$watch('$wire.data.otp', value => otpValid = value && value.length === 6)" :disabled="!otpValid" :class="otpValid ? 
                                    ...
                                    'bg-gray-400 cursor-not-allowed'" class="inline-flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-white border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200">
